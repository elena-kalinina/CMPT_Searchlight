# -*- coding: utf-8 -*-
"""
Created on Thu Jun 30 17:17:52 2016
    ...
    def __call__(self, imgs):
      ...
      #  signals = np.empty(imgs.shape[3], n_seeds)
        for i, sphere in enumerate(_iter_signals_from_spheres(
            ...
            signals = np.empty((imgs.shape[3], sphere.shape[1], n_seeds))
         #   print sphere.shape
         #   print signals.shape
            signals[:, :, i] = sphere #np.mean(sphere, axis=1)

        return signals, None


class myNiftiSpheresMasker(BaseMasker, CacheMixin):
    ...
    def transform_single_imgs(self, imgs, confounds=None):
        ...
        params = get_params(myNiftiSpheresMasker, self)
