...
n_jobs=8    
for vox in range(0, len(mask_index[0])):
   alert, stat_map[vox], all_maps_1, all_maps_2 = my_cmpt_searchlight(n_of_subj, subj_data_all_1, subj_data_all_2, labels_all_1, labels_all_2, mask_index, vox, gm_mask)
   print stat_map[vox]
   if alert==0:
       #sign_map[vox]=sl_cmpt_permutations(labels_all_1, labels_all_2, all_maps_1, all_maps_2, n_of_subj, stat_map[vox])
       result = Parallel(n_jobs=n_jobs)(delayed(sl_cmpt_permutations)(labels_all_1, labels_all_2, all_maps_1, all_maps_2, n_of_subj, stat_map[vox]) for perm in range(0, n_perm)) #len(mask_index[0]))) #len(mask_index[0])))
       print np.sum(result)
######################################
...
niimg=main_masker.inverse_transform(sign_map.T)
