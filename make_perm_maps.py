...
def sl_cmpt_permutations_new(subjects, masker, nperm, stat_true):
    for s in subjects:
        fname11='beta_'+SubjName[subj]+'_'+str(nperm)+'_Perc'+'_cond0.nii'
        fname12='beta_'+SubjName[subj]+'_'+str(nperm)+'_Perc'+'_cond1.nii'
        fname21='beta_'+SubjName[subj]+'_'+str(nperm)+'_Im'+'_cond0.nii'
        fname21='beta_'+SubjName[subj]+'_'+str(nperm)+'_Im'+'_cond1.nii'
        img_cond_modality_1=[0]*2
        img_cond_modality_2=[0]*2
       
        stat_perm=0
        
        img_cond_modality_1[0]==masker.fit_transform(nib.load(fname11))
        img_cond_modality_1[1]=masker.fit_transform(nib.load(fname12))
        img_cond_modality_2[0]=masker.fit_transform(nib.load(fname21))
        img_cond_modality_2[1]=masker.fit_transform(nib.load(fname22))
...
results_dir="/home/elena/ATTEND/cross_modal/sl_res/test_1"
...
#        print [labels1_perm==conditions[0]]
        for subj in range(0, n_of_subj): 
                ...
                temp_array2_0=np.array((temp_array2[labels1_perm==conditions[0]]).mean(axis=0), dtype='f4')
                temp_array2_1=np.array((temp_array2[labels1_perm==conditions[1]]).mean(axis=0), dtype='f4')
                ...
                nib.save(nii1, 'beta_'+SubjName[subj]+'_'+str(perm)+'_Perc'+'_cond0.nii.gz')
                nib.save(nii2, 'beta_'+SubjName[subj]+'_'+str(perm)+'_Perc'+'_cond1.nii.gz')
                nib.save(nii3, 'beta_'+SubjName[subj]+'_'+str(perm)+'_Im'+'_cond0.nii.gz')
                nib.save(nii4, 'beta_'+SubjName[subj]+'_'+str(perm)+'_Im'+'_cond1.nii.gz')
